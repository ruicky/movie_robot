...
2. 优化部分站点提示120秒访问过载时程序处理逻辑，保证最终正常获取结果；
3. 优化处理智能检索和下载任务的线程模型，彻底杜绝对PT站点访问时的并发，降低站点访问压力，避免被ban；
4. 优化养站拉取新种子的逻辑，如果最近5秒持续上行速度超过设定上行带宽峰值的85%，则停止拉新种子，以此提高效率，降低硬盘损耗；
5. 优化锁的应用方式，部分场景改为无锁编程，所有加锁场景改为非阻塞方式，减少竞争带来的开销；
在短时间大量下载电影和剧集时，此版本优化将使程序更稳定。其绝大多数优化场景都是针对智能下载（各种订阅的处理），pt站点频繁挂，通过任何方式手动下载都是痛苦的，所以这里鼓励大家使用智能下载，避免种子选择，和站点挂掉时的等待。
